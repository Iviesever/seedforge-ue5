...
The user supplied the career/product goal, deadline, repository/engine boundaries, agent/process limits, non-goals, acceptance gates, authority for branch commits/push/Draft PR, and the requirement for honest evidence. Codex selected implementation details, wrote C++/Automation tests/PowerShell/docs, ran UE 5.8 and Git/GitHub workflows, investigated failures, inspected screenshots, and assembled local artifacts.
...
> I scoped and orchestrated an AI-assisted UE 5.8 C++ project that turns deterministic dungeon generation into a packaged extraction vertical slice. I can reproduce its 63-test and package evidence, explain the encounter/A*/state/async ownership boundaries, and use its live-change drills to build personal implementation skill.

After the user personally studies and modifies the system, they may additionally describe that specific personally authored change and the understanding demonstrated by it.
...
1. Reproduce build, 63-test Automation, Editor gameplay smoke, BuildPlugin, BuildCookRun, ordinary packaged launch, and packaged smoke.
