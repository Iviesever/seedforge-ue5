...
UAT's initial run selected its installed-engine AutomationTool `Saved` directory for internal diagnostic files despite all project-facing outputs being under SeedForge. No Engine file was edited, removed, or committed manually. `PackagePlugin.ps1` and `PackageDemo.ps1` now set `uebp_LogFolder`, `uebp_FinalLogFolder`, and `uebp_EngineSavedFolder` to unique project-local `Artifacts/Logs/uat-*` directories; the final PACT-34 verification proves the complete redirection.
